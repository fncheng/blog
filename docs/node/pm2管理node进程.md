...
[pm2中文网](https://pm2.fenxianglu.cn/docs/general/process-management)
...
# pm2 start with name
pm2 start app.js --name "my-pm2-app"
```
...
```sh
pm2 --name <NAME> start /home/cheng/zk8/src/index.js
```
...
1.crontab配置pm2时报错`pm2 node no such file or directory`

原因是bash找不到node bin程序

解决方案：ln -s /usr/bin/nodejs /usr/bin/node

如果node是用nvm安装的话，就是 ` ln -s $NVM_DIR/versions/node/v15.14.0/bin/node /usr/bin/node`
